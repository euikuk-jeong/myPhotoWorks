...
### Known limitations
- 그룹핑 기본값(유사도 임계값 등)과 촬영 시각 신뢰 판정은 아직 실제 사진(필름 스캔·연사 등)으로 튜닝하지 않았습니다. 결과가 맞지 않으면 그룹 탭의 유사도 슬라이더와 리뷰 창의 그룹 수정으로 조정하세요.
- 선명도·노출·색감 점수는 사진 전체 기준이라 주제·초점 영역, 인물, 구도, 눈 감음은 반영하지 않습니다. 약한 블러는 "흐림" 배지로 표시되지 않을 수 있습니다.
